...
void DG_Init() {
  ...
  // A better way to do this would be to mmap it into user space, but we need ipc first
#define VGA_FRAMEBUFFER 0xffffff8004000000
  fbPtr = (uint8_t*) VGA_FRAMEBUFFER;
	// clear the screen
	memset(fbPtr, 0, fbStride * fbHeight);
	
	// get the start time
	timestamp = execute_syscall(SYS_GETUPTIME, 0, 0, 0);
}
...
void DG_SleepMs(uint32_t ms) {
  // TODO: implement sleep through the libc
  execute_syscall(SYS_NANOSLEEP, ms * 1000000, 0, 0);
}
...
uint32_t DG_GetTicksMs() {
  ...
  uint64_t current_time = execute_syscall(SYS_GETUPTIME, 0, 0, 0);
  return (current_time - timestamp) / 1000000;
}
